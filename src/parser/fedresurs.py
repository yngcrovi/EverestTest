...
class FedResursParser:
    ...
    async def get_data_about_bankruptcy(self, data: list[dict]) -> dict:
        bankruptcy_data: list[dict] = []

        for i, el in enumerate(data):
            # Переходим к профилю человека
            await self.page.goto(f"{self.url + el['path']}", wait_until='networkidle')
            case_num: str = None
            last_dt: str = None

            # Собираем все боковые кнопки
            div_bankrupt = self.page.locator('div[page-item-name="bankrupt"]')
            div_case_num = div_bankrupt.first

            div_case_num = div_bankrupt.locator('div.info-item-name_properties')

            if await div_case_num.count():
                div_case_num = await div_case_num.locator('a').all()
                if len(div_case_num) > 0:
                    case_num = await div_case_num[0].text_content()

            div_last_dt = await div_bankrupt.locator('div.info-item-value').all()

            if len(div_last_dt) > 0:
                div_case: list[Page] = await div_last_dt[0].locator('entity-card-bankruptcy-publication-wrapper').all()
                div_dt = div_case[0].locator('div.pub-link').locator('a')
                if await div_dt.count():
                    info = await div_dt.text_content()
                    import re
            ...
            if case_num and last_dt:
                bankruptcy_data.append({
                    'inn': el['id'],
                    'case_num': case_num, 
                    'last_bankruptcy_dt': datetime.strptime(last_dt, "%Y-%m-%d").date()
                })
            print('✅ Выполнено:', el['fio'], f'({i+1}/{len(data)})')
        print(bankruptcy_data)
        print('🔄 Сохраняем данные в таблицу bankruptcy_info...')

        await BankruptcyInfoService().insert_data(bankruptcy_data)

        print('✅ Данные успешно сохранены в таблицу bankruptcy_info')

        return bankruptcy_data
    

    # async def get_data_about_bankruptcy_parallel(self, data: list[dict], max_concurrent=5) -> dict:
    #     """
    #     Параллельный парсинг с сохранением исходной логики
    #     max_concurrent - количество одновременно открытых вкладок
    #     """
        
    #     # Общий список для результатов
    #     all_results = []
        
    #     # Семафор для ограничения количества одновременных вкладок
    ...
    #         """Обработка одного человека в отдельной вкладке"""
    ...
    #             # Поиск номера дела (полностью как в оригинале)
    #             div_case_num = div_bankrupt.locator('div.info-item-name_properties')
    #             if await div_case_num.count():
    #                 div_case_num_elements = await div_case_num.locator('a').all()
    #                 if len(div_case_num_elements) > 0:
    #                     case_num = await div_case_num_elements[0].text_content()

    #             # Поиск даты (полностью как в оригинале)
    #             div_last_dt_elements = await div_bankrupt.locator('div.info-item-value').all()
    #             if len(div_last_dt_elements) > 0:
    #                 div_case = await div_last_dt_elements[0].locator('entity-card-bankruptcy-publication-wrapper').all()
    #                 if len(div_case) > 0:
    #                     div_dt = div_case[0].locator('div.pub-link').locator('a')
    #                     if await div_dt.count():
    #                         info = await div_dt.text_content()
    ...
    #                     print(f'✅ Выполнено: {person_data["fio"]} ({index + 1}/{len(data)})')
    #                     return {
    #                         'inn': person_data['id'],
    #                         'case_num': case_num,
    ...
    #                     }
    #                 except ValueError:
    #                     print(f'⚠️ Ошибка формата даты: {last_dt} для {person_data["fio"]}')
    #                     return None
    #             else:
    #                 print(f'⚠️ Пропущено: {person_data["fio"]} (нет данных)')
    #                 return None
                    
    #         except Exception as e:
    #             print(f'❌ Ошибка при обработке {person_data["fio"]}: {str(e)[:100]}')
    #             return None
    #         finally:
    #             # Важно закрыть вкладку
    #             await page.close()
        
    #     # Функция для запуска с семафором
    ...
    #     # Запускаем задачи и собираем результаты по мере выполнения
    #     for completed in asyncio.as_completed(tasks):
    #         result = await completed
    #         if result:
    #             all_results.append(result)
    #             print(f'📊 Прогресс: {len(all_results)}/{len(data)}')
        
    #     print(f'\n🔍 Найдено {len(all_results)} записей из {len(data)}')
    #     print(all_results)
    ...
    #         print(f'✅ Данные успешно сохранены ({len(all_results)} записей)')
    #     else:
